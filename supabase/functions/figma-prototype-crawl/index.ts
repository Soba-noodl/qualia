...
async function processPrototypeCrawl(params: {
  ...
  try {
    ...
    // Retry up to 3 times to handle cold-start 500s and transient Edge Function errors.
    const supabaseUrl = getSupabaseUrl();
    const serviceKey = getSecretKey();
    let triggered = false;
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        const triggerResp = await fetch(`${supabaseUrl}/functions/v1/prototype-analyze`, {
          method: "POST",
          headers: {
            "Authorization": `Bearer ${serviceKey}`,
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ audit_id: auditId, ...(reqProvider ? { provider: reqProvider } : {}), ...(reqModel ? { model: reqModel } : {}) }),
        });
        if (triggerResp.ok) {
        ...
        console.warn(`[prototype-crawl] prototype-analyze attempt ${attempt} returned ${triggerResp.status}: ${text.slice(0, 200)}`);
      } catch (err) {
        console.warn(`[prototype-crawl] prototype-analyze attempt ${attempt} threw:`, err instanceof Error ? err.message : err);
      }
      if (attempt < 3) await sleep(Math.pow(2, attempt) * 1000);
    }
    if (!triggered) {
      await serviceClient.from("audits")
        .update({ status: "failed", completed_at: new Date().toISOString(), error_message: "Export succeeded but failed to start AI analysis after 3 attempts. Please retry." })
        .eq("id", auditId);
    }
  } catch (err) {
