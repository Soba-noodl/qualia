...
serve(async (req) => {
  ...
  try {
    ...
    // T-080: keep `keptStoragePaths` in lockstep with `signedUrls` so the Anthropic
    // adapter can resize each frame by its source path.
    const signedUrls: string[] = [];
    const keptStoragePaths: string[] = [];
    ...
    try {
      analysisJson = await callAiAndParse(
        ...
        isFlowMode,
        {
          ...
          // T-080: storage paths in the same order as signedUrls (1:1 — failed signings
          // were excluded above so both arrays stay aligned).
          imageStoragePaths: keptStoragePaths,
        },
