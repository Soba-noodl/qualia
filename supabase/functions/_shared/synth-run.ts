...
async function callGeminiSynthRaw(
  ...
  const result = await runLLM({
    ...
    // T-080: only consumed by the Anthropic adapter for multi-image resize.
    imageStoragePaths,
  });
...
async function callGeminiForSynthBatch(
  ...
  const parsed = await callGeminiSynthRaw(systemPrompt, userInstruction, imageUrls, opts, 9000, auditId, imageStoragePaths);
...
function callGeminiForSynth(
  ...
  return callGeminiSynthRaw(systemPrompt, userInstruction, imageUrls, opts, 3000, auditId, imageStoragePaths);
}
...
export async function runSynthAnalysis(params: {
  ...
  personaIds: string[];
  imageUrls: string[];
  /** T-080: storage paths aligned 1:1 with imageUrls (Anthropic resize). */
  imageStoragePaths?: string[];
  ...
  const { userId, isTrialEligible, requestedProvider, requestedModel, promptVersion, personaIds, imageUrls, imageStoragePaths, projectMission, screenContext, projectLanguage, auditId } = params;
  ...
  try {
    const batchResults = await callGeminiForSynthBatch(personaIds, projectContext, imageUrls, synthOpts, isFlow, auditId, imageStoragePaths);
  ...
  const results = await Promise.all(
    personaIds.map(async (personaId) => {
      try {
        ...
        const raw = await callGeminiForSynth(systemPrompt, imageUrls, synthOpts, isFlow, auditId, imageStoragePaths);
        return mapToSynthResult(personaId, raw);
      } catch (err) {
