 ...
 *  Anthropic's OpenAI-compat layer ignores response_format=json_object, so its
 *  models often emit fenced JSON. Also handles leading prose like "Here's the
 *  JSON:\n\n```json{...}```" by trimming everything before the first { and
 *  after the last }.
 */
...
export interface CallAiAndParseOpts {
  ...
  /**
   * T-080: storage paths aligned 1:1 with `imageUrls` / `contextUrls`. Needed
   * by the Anthropic adapter to server-side resize multi-image inputs.
   * Gemini / OpenAI ignore these.
   */
...
export async function callAiAndParse(
  ...
  const result = await runLLM({
    ...
    contextUrls,
    imageLabels,
    // Token budgets tuned 2026-05-20 after live.com flow audit on Haiku 4.5 hit
    // the cap at exactly 8000 tokens (truncated JSON → parse failure). Bumped
    // generously so prose-heavy models (Anthropic, OpenAI mini) have headroom.
    // Cost impact at the cap: Haiku ~$0.08, Opus ~$0.40, GPT-5.4 ~$0.24 — acceptable.
    maxTokens: maxTokensOverride ?? (isFlowMode ? 16000 : 6000),
    maxAttempts,
    timeoutMs,
    auditId,
    promptVersion: opts.promptVersion,
    // T-080: forward storage paths for Anthropic resize. Other providers ignore.
    imageStoragePaths: opts.imageStoragePaths,
    contextStoragePaths: opts.contextStoragePaths,
  });
