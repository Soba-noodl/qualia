...
export interface RunLLMInput {
   ...
   * `imageUrls`. Required by the Anthropic adapter for multi-image audits so
   * it can server-side resize to ≤ 2000px. Gemini and OpenAI ignore these.
   */
...
export async function runLLM(input: RunLLMInput): Promise<RunLLMResult> {
  ...
  const callArgs = {
    ...
    // T-080: Anthropic-only fields. Gemini / OpenAI adapter signatures accept
    // extra properties and discard them (TypeScript structural typing — passing
    // a wider object to a function expecting a narrower one is fine).
    imageStoragePaths: input.imageStoragePaths,
    contextStoragePaths: input.contextStoragePaths,
  ...
  try {
    ...
    else if (resolved.provider === "anthropic") result = await callAnthropic(callArgs);
