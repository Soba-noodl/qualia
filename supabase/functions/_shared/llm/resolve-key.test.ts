...
Deno.test("resolveKey: user has BYOK key → uses it, no trial consumption", async () => {
  ...
  // We use a real encrypt to produce the ciphertext that decrypt() must be able to recover.
  // But for unit test, we'll mock decrypt by importing it… simpler: import encrypt and produce a real one.
  const { encrypt } = await import("../encryption.ts");
  const encrypted = await encrypt("sk-ant-realkey", "x".repeat(32));

  const supabase = makeFakeSupabase({
    user_llm_keys: [{ user_id: "u1", provider: "anthropic", encrypted_key: encrypted, model_override: null }],
    profiles: [{ user_id: "u1", free_analysis_used_at: null, default_llm_provider: "anthropic" }],
  });
  ...
  assertEquals(r.apiKey, "sk-ant-realkey");
});
...
Deno.test("resolveKey: user with BYOK key (regardless of admin status) → uses their key", async () => {
  ...
  Deno.env.set("INTEGRATION_ENCRYPTION_KEY", "x".repeat(32));
  const { encrypt } = await import("../encryption.ts");
  const encrypted = await encrypt("sk-ant-user-key", "x".repeat(32));
  const supabase = makeFakeSupabase({
    user_llm_keys: [{ user_id: "u2", provider: "anthropic", encrypted_key: encrypted, model_override: null }],
    profiles: [{ user_id: "u2", free_analysis_used_at: null, default_llm_provider: "anthropic" }],
  });
  // deno-lint-ignore no-explicit-any
...
Deno.test("resolveKey: requestedModel overrides saved model_override for BYOK key", async () => {
  ...
  const supabase = makeFakeSupabase({
    user_llm_keys: [{ user_id: "u1", provider: "anthropic", encrypted_key: encrypted, model_override: "claude-opus-4-7" }],
    profiles: [{ user_id: "u1", free_analysis_used_at: null, default_llm_provider: "anthropic" }],
  });
  // deno-lint-ignore no-explicit-any
  const r = await resolveKey({ supabase: supabase as any, userId: "u1", isTrialEligible: false, requestedModel: "claude-haiku-4-5", encryptionKey: "x".repeat(32) });
  assertEquals(r.model, "claude-haiku-4-5");
  assertEquals(r.paidBy, "user");
});
