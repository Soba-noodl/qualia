...
import { AA_CONTRAST_THRESHOLD, buildFlowContrastSection, type ContrastResult } from "../_shared/contrast-section.ts";
...
/**
 * Download each signed object once; build data: URLs for Gemini unless total size exceeds cap or any download fails.
 */
async function resolveGeminiImageStrings(
  serviceClient: SupabaseClient,
  signedUrls: string[],
): Promise<{ urls: string[]; inlineSkipped: boolean }> {
  if (signedUrls.length === 0) return { urls: [], inlineSkipped: false };
  ...
  for (const signedUrl of signedUrls) {
    try {
      const { bucket, objectPath } = parseBucketPath(signedUrl);
      const { data, error } = await serviceClient.storage.from(bucket).download(objectPath);
      if (error || !data) {
        console.warn("[analyze-ui] storage download failed, using signed URLs for Gemini:", error?.message ?? "no data");
      ...
      console.warn("[analyze-ui] download error, using signed URLs for Gemini:", e);
      return { urls: [...signedUrls], inlineSkipped: true };
    }
  }

  if (total > INLINE_CAP_BYTES) {
    console.info("[analyze-ui] inline_skipped: true reason=total_bytes_over_cap", { total, cap: INLINE_CAP_BYTES });
    return { urls: [...signedUrls], inlineSkipped: true };
  }

  const urls = buffers.map((bytes) => {
    const mime = sniffImageMime(bytes);
    return `data:${mime};base64,${base64Encode(bytes)}`;
  });
  return { urls, inlineSkipped: false };
}
...
serve(async (req) => {
  ...
  try {
    ...
    const {
      ...
      provider, // Optional BYOK provider override: "gemini" | "anthropic" | "openai"
    ...
    // T-080: derive storage paths from signed URLs for the Anthropic resize path.
    // Falls back to undefined when any URL isn't a `screenshots` signed URL (e.g.
    // legacy external URL); Anthropic multi-image then surfaces a friendly error.
    function deriveStoragePath(signedUrl: string): string | null {
      try {
        const { bucket, objectPath } = parseBucketPath(signedUrl);
        if (bucket !== "screenshots") return null;
        return objectPath;
      } catch {
        return null;
      }
    }
    const imageStoragePathsForResize = imageUrls.map(deriveStoragePath);
    const contextStoragePathsForResize = contextUrls.map(deriveStoragePath);
    const allImagePathsResolved = imageStoragePathsForResize.every((p): p is string => p !== null);
    const allContextPathsResolved = contextStoragePathsForResize.every((p): p is string => p !== null);
    const anthropicStoragePaths = allImagePathsResolved && allContextPathsResolved
      ? {
          imageStoragePaths: imageStoragePathsForResize as string[],
          contextStoragePaths: contextStoragePathsForResize as string[],
        }
      : null;
    if (inlineSkipped) {
      console.info("[analyze-ui] Gemini image input: signed URLs (inline skipped or fallback)");
    } else {
      console.info("[analyze-ui] Gemini image input: data URLs (service download once per image)");
    }
...
4. NEVER HALLUCINATE: If ratio >= 4.5, do NOT complain about contrast.`;
      ...
      const perStep = await Promise.all(imageUrls.map((url) => fetchContrast(url)));
      const results: ContrastResult[] = perStep.map((res, idx) => {
        if (!res.ok) {
          return {
            image_index: idx,
            ratio: null,
            ...
            note: "contrast calculation failed",
          };
        }
        const data = res.data;
        return {
          ...
          ratio: data.ratio,
          level: data.level,
          foreground_hex: data.foreground_hex ?? "",
          background_hex: data.background_hex ?? "",
          ...(data.note ? { note: data.note } : {}),
        };
      });
      contrastDataSection = buildFlowContrastSection(results);
    }
    ...
    try {
      analysisJson = await callAiAndParse(
        ...
        isFlowMode,
        {
          ...
          imageStoragePaths: anthropicStoragePaths?.imageStoragePaths,
          contextStoragePaths: anthropicStoragePaths?.contextStoragePaths,
        },
        undefined,
        undefined,
        undefined,
