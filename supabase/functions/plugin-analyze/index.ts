...
// 10 plugin analyses per user per minute — same cap as analyze-ui. In-memory
// state, per-instance; effective ceiling is `instances × max`. The BYOK quota
// gate upstream is the source of truth for billing limits.
const pluginAnalyzeRateLimiter = new RateLimiter({ windowMs: 60_000, max: 10 });
