...
serve(async (req) => {
  ...
  try {
    ...
    // T-080: keep storage paths aligned with the signed URLs so Anthropic resize works.
    ...
    const keptStoragePaths: string[] = keptPairs.map((entry) => entry.path);
    ...
    try {
      synthBlock = await runSynthAnalysis({
        ...
        imageStoragePaths: keptStoragePaths, // T-080: Anthropic resize path
        projectMission,
