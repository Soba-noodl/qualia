...
async function runAnalysis(params: {
  ...
  try {
    ...
    // T-080: track kept paths alongside signedUrls so the Anthropic adapter can
    // resize each frame from its source path.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { audit_id, provider, model } = await req.json() as { audit_id?: string; provider?: "gemini" | "anthropic" | "openai"; model?: string };
    if (!audit_id) {
