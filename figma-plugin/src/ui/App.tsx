...
export default function App() {
  ...
  useEffect(() => {
    ...
    // NOTE: a strict `event.source !== window.parent` guard was added here
    // 2026-05-25 then reverted 2026-05-26 — Figma's sandboxed iframe runtime
    // doesn't always make event.source === window.parent, and rejecting
    // mismatches breaks the entire plugin↔sandbox channel. The existing
    // `event.data.pluginMessage` shape check in onMessage is the actual
    // working contract; rely on that + Figma's null-origin sandboxing.
