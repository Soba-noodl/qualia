...
# Version tag for this release. REQUIRED for --push: a hardcoded default
# once silently re-published v10.1 on top of a later v10.2 release (the
# default went stale and was never bumped). For dry-run/inspect a throwaway
# placeholder is fine since nothing leaves the machine.
if [[ -z "${PUBLIC_VERSION:-}" ]]; then
  if [[ "$MODE" == "push" ]]; then
    latest=$(git ls-remote --tags "$PUSH_URL" 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+' | sort -V | tail -1)
    echo "Error: PUBLIC_VERSION must be set for --push (e.g. PUBLIC_VERSION=v10.3 ...)." >&2
    echo "       Latest public tag: ${latest:-none}" >&2
    exit 1
  fi
  PUBLIC_VERSION="v0.0-dryrun"
fi
