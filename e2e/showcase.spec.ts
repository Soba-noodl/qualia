 ...
 * name (Gemini / GPT). The AI provider is only surfaced as an icon on the
 * landing hero via `BrandLogo`, never as text on showcase cards.
 */
...
test.describe('Showcase page (unauthenticated)', () => {
  ...
  // Test #5 from spec: "Cards display the provider name (Gemini / GPT)"
  //
  ...
  test.skip('cards display the provider name (Gemini / GPT)', async () => {
