              ...
              {imageUrls.length > 0 && (
                <div className={isFlowMode ? "lg:col-span-2" : ""}>
                  <div className={isFlowMode ? "lg:sticky lg:top-24 space-y-6 h-fit" : "space-y-6"}>
                    ...
                    <AuditContextCard
                      screenGoal={row.screen_context || null}
                      isFlowAudit={isFlowMode}
                      mission={row.project_mission}
                      personas={(row.selected_personas ?? []).map((p, i) => ({
                        id: `persona-${i}`,
                        name: p.name,
                        description: p.description,
                      }))}
                      selectedPersonaNames={(row.selected_personas ?? []).map((p) => p.name)}
                      constraints={null}
                      contextImages={null}
                    />
                  </div>
