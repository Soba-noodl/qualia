...
or the cron jobs and triggers will return 401. The two stores (Postgres
`private.cron_config` row + Edge Function env var) have no foreign key
between them — keep them in sync manually, and smoke-test by manually
firing the cron command body via `SELECT net.http_post(...)` and asserting
`status_code = 200` in `net._http_response`.
